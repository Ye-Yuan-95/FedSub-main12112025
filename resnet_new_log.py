...
def parse_args(args, remaining_args):
    ...
    "--sanity_exp1",
    action="store_true",
    help="Sanity check: fixed P, no lambda, no outer_update"
    )
    parser.add_argument(
    "--fix_P",
    action="store_true",
    help="Fix compression matrix P: never regenerate new P (gene_new_cp=False)."
    )    
    # data
    ...
    "--use_alg4",
    action="store_true",
    help="Enable Algorithm 4: use global momentum m instead of lambda"
    )
    parser.add_argument(
    "--use_alg5",
    action="store_true",
    help="Enable Algorithm 5 variant inside SubScafSGD (no new optimizer name)."
    )
    # log
...
def main(args):
    ...
    log(
        f"DATA CHECK | "
        f"train_len={len(train_loader.dataset)} "
        f"val_len={len(val_loader.dataset)} "
        f"data_hete={args.data_hete}"
    )
    ...
    if args.evaluate:
        validate(val_loader, model, criterion, args, device, epoch=1)

    for epoch in range(args.epochs):
    # === ADD: epoch header (rank0 only) ===
        if dist.get_rank() == 0:
            log(f"Epoch: [{epoch+1}/{args.epochs}] START")
        # train for one epoch
        ...
        if args.evaluate:
            ...
            prec1,prec5 = validate(val_loader, model, criterion, args, device, epoch)

            # remember best prec@1 and save checkpoint
            best_prec1 = max(prec1, best_prec1)
              # === ADD: epoch footer summary (rank0 only) ===
            if dist.get_rank() == 0:
                log(f"Epoch: [{epoch+1}/{args.epochs}] END | Prec@1={prec1:.3f} | BestPrec@1={best_prec1:.3f} | Prec@5={prec5:.3f}")
...
def train(train_loader, model, criterion, optimizer, epoch, schedule, args, device, *arg):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if "subscaf" in args.optimizer.lower() and update_step % args.tau == 0:
            if getattr(args,"sanity_exp1",False):
                pass
            else:

                if getattr(args, "fix_P", False):
                    gene_new_cp = False
                else:
                    gene_new_cp = (update_step % args.update_cp_freq == 0)
                if dist.get_rank() == 0 and update_step % args.tau == 0:
                    log(f"DBG P update | fix_P={getattr(args,'fix_P',False)} gene_new_cp={gene_new_cp}")
                if args.measure_comm:
                    ...
                    outer_update(model, lbd, comp_mat_rec, target_modules_list, optimizer, 
                                    subscaf_params, args, device, jump_modules_list, gene_new_cp, layer='conv2d')
                if dist.get_rank() == 0 and getattr(args, "use_alg4_momentum", False):
                    log(f"ALG4 | set m_global from avg_b/tau | norm={m_global.norm().item():.4e}")
                if dist.get_rank() == 0 and update_step % args.tau == 0:
                    log(f"DBG ALG switch | use_alg4={getattr(args,'use_alg4',False)} use_alg5={getattr(args,'use_alg5',False)} fix_P={getattr(args,'fix_P',False)}")
                if args.measure_comm:
        ...
        prec5 = accuracy(output.data, target, (1,5)) [1]
    ...
    if dist.get_rank() == 0:
        if "subscaf" in args.optimizer.lower():
            log_subscaf_stats(model, lbd, epoch, tag=args.optimizer)
        log(
            'Epoch: [{0}/{1}] TRAIN_END '
            '| AvgLoss={loss:.4f} '
            '| Prec@1={prec1:.3f} '
    ...
    if dist.get_rank() == 0 and args.use_log and (epoch % 5 == 0):
    # 用当前 train_loader 的 dataset（确保和训练同一个数据源）
        ...
        prec1_train_eval, prec5_train_eval = validate(
            train_eval_loader, model, criterion, args, device, epoch
        )
        log(f"TRAIN_EVAL (model.eval on train set) | Prec@1={prec1_train_eval:.3f} | Prec@5={prec5_train_eval:.3f}")
def validate(val_loader, model, criterion, args, device, epoch):
    """
    Run evaluation
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            prec5 = accuracy(output.data, target,topk=[1,5])[1]
    ...
    log(' * Prec@5 {top5.avg:.3f}'.format(top5=top5))
    # 追加 epoch 级 TEST_END 日志（rank0 才打）
    if dist.get_rank() == 0 and args.use_log:
        log(
            'Epoch: [{0}/{1}] TEST_END '
            '| AvgLoss={loss:.4f} '
            '| Prec@1={prec1:.3f} '
            '| Prec@5={prec5:.3f}'.format(
                epoch, args.epochs,
                loss=losses.avg,
                prec1=top1.avg,   # 目前你的 accuracy() 只算了 top1，这里先当作 Prec@1
                prec5=top5.avg    # 如果以后你算 top5，再拆成两个
            )
        )
...
def log_subscaf_stats(model, lbd, epoch, tag=""):
    lbd_norm = 0.0
    for l in lbd:
        lbd_norm += l.norm().item()
    lbd_norm /= max(len(lbd), 1)

    b_norm, x_norm, n_layers = 0.0, 0.0, 0
    for m in model.modules():
        if hasattr(m, "b") and hasattr(m, "x"):
            b_norm += m.b.norm().item()
            x_norm += m.x.norm().item()
            n_layers += 1
